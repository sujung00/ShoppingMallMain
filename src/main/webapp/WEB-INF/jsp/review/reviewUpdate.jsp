...
<div class="container pt-5">
	<div class="d-flex justify-content-around">
		...
		<div class="product-detail">
			<div>
				<input type="text" name="subject" placeholder="제목" class="form-control" value="${review.subject}">
			</div>
			<div class="mt-3">
				<textarea rows="10" id="content" placeholder="리뷰 내용을 작성해주세요." class="form-control">${review.content}</textarea>
			</div>
			...
			<div class="mt-4">
				<button type="button" id="reviewUpdateBtn" class="btn review-btn2" 
				data-user-id="${reviewView.userId}" data-review-id="${review.id}">수정 완료</button>
			</div>
		</div>
	</div>
</div>

<div class="modal" tabindex="-1" id="reviewUpdateModal">
		...
		$.ajax({
			...
			, success:function(data){
				if(data.code == 1){
					...
					$('#reviewUpdateModal').on('hidden.bs.modal', function(e) {
			...
			, error : function(request, status, error) {
				$("#reviewUpdateModal").modal();
				$("#modalBody").text("리뷰를 수정하는데 실패했습니다.");
				return;
			}
		})
	});
